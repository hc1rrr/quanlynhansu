fetch("sidebar.html")
  ...
  document.addEventListener("DOMContentLoaded", function () {
    fetch("../php/employee.php?action=get") // Gọi API lấy dữ liệu nhân viên từ PHP
        ...
        .then(data => {
            if (data.success) {
                ...
                data.data.forEach(emp => {
                    const row = `
                        <tr>
                            ...
                            <td>${emp.Luong.toLocaleString()}</td>
                            <td>
                ...
                console.error("Lỗi lấy danh sách nhân viên: ", data.message);
            }
        })
...
function addDepartment() {
    console.log("Hàm addDepartment() đã được gọi!");
    alert("Chức năng thêm phòng ban chưa được triển khai!");
}
